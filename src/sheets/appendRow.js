...
function createAppendRow(options = {}) {
  ...
  const rowBuilder =
    typeof options.rowBuilder === "function" ? options.rowBuilder : buildRowFromRideObject;
  ...
  return async function appendRow(ride) {
    ...
    const row = rowBuilder(ride, headers);
    ...
    try {
      ...
      logger.debug("Row appended to Google Sheet", {
        stage: "sheets_append",
        fallbackUsed: false,
        attempt: lastAttempt,
